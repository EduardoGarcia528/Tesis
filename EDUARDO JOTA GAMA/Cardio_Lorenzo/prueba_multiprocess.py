...
usar_archivos_guardados = True
method = 'lineal'
size = 0
guardar_png_por_letra = True
MS =  3.0
indice_gamma = 1
...
def procesar_hoja(letra):
    ...
    if usar_archivos_guardados and os.path.exists(archivo_J):
    ...
    if os.path.exists(archivo_gamma) and usar_archivos_guardados:
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.savefig(f"correlacion_por_columna_method-{method}_size-{size}_gamma-{indice_gamma}_MS-{MS}.png")
    plt.close()
    ...
    plt.ylabel("Coeficiente de correlación")
    plt.tight_layout()
    plt.savefig(f"correlacion_por_paciente_method-{method}_size-{size}_gamma-{indice_gamma}_MS-{MS}.png")
    plt.close()
