...
METHOD = "hybrid"  # "hybrid" o "complexity"
# =========================================================
...
def save_panel_measure_csv(df_panel, composer_names, cfg, method,export_dir=EXPORT_DIR):
...
def build_cache_key(measure,D, tau, type_null, normalize, alternative, method):
    if "PE" in measure:
        return f"{measure}_D{D}_tau{tau}_{type_null}_{alternative}_{method}"
    elif "Cd" in measure:
        return f"{measure}_d{D+1}_mu{tau}_{type_null}_{alternative}_{method}"
    elif "gamma" in measure:  
        return f"{measure}_d{D}_mu{tau}_{type_null}_{alternative}_{method}"
    elif "H_orbit" in measure:
        return f"{measure}_d{D}_{type_null}_{alternative}_{method}"
    elif "H_tau" in measure:
        return f"{measure}_tau{tau}_{type_null}_{alternative}_{method}"
    else:
        return f"{measure}_{type_null}_{alternative}_{method}"

def get_cache_path(measure,cache_dir, D, tau, type_null, normalize, alternative, method):
    os.makedirs(cache_dir, exist_ok=True)
    fname = build_cache_key(measure,D, tau, type_null, normalize, alternative, method) + ".pkl"
...
# Cargar composers
with open(os.path.join(carpeta_salida, f"composers_{METHOD}.pkl"), "rb") as f:
    composers = pickle.load(f)

# Cargar datos_composers
with open(os.path.join(carpeta_salida, f"datos_composers_{METHOD}.pkl"), "rb") as f:
