...
r_values = np.sort(np.append(np.linspace(3.0, 4.0, 24), np.array([3.569945672])))    # puedes aumentar esto
...
for i, r in enumerate(r_values):
    ...
    C = []
    for j in range(1,16):
        pe = permutation_entropy(serie,m=5,tau=j)
        C.append(pe)
    ...
    ax.plot(d[1:], C[:], marker='o', linestyle='-', linewidth=1.0, markersize=2.5)
