...
def bifurcacion_con_J(
    ...
    for var_ruido in ruidos:
        ...
        for _ in range(10):
            ...
            for r in np.linspace(r_min, r_max, resolucion_r):
                ...
                # J = entropia_shannon(serie,False)
                _, J = gamma_index_jacobs_rank(angulos,max_gamma=1, mu=2)
                r_vals_J.append(r)
                J_vals.append(J[0])
    ...
    if graficar:
        ...
        # Índice J
        ax2 = ax1.twinx()
        # ax2.plot(r_vals_J, J_vals, color='black', alpha = 1, label = 'PE sin ruido')
        for i,var_ruido in enumerate(ruidos):
            # medida_por_ruido.append(np.load(f'data/RUIDOS/iterativo_PE/PE_por_ruido_{str(var_ruido)}.npy'))
            ax2.plot(r_vals_J, medida_por_ruido[i], color=colores[i] , alpha = 1, label= f'Varianza = {var_ruido}')
            # ax2.annotate(f'Varianza = {ruido}', xy=(r_vals_J[len(r_vals_J)//4], medida_por_ruido[i][len(r_vals_J)//4]), xytext=(10, medida_por_ruido[i][-1]),
