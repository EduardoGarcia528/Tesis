...
def relabel(order):
    # re-etiqueta a 0..k-1 preservando el orden relativo
    rank = {v:i for i,v in enumerate(sorted(order))}
    return tuple(rank[v] for v in order)

def border_first(pattern, m, tau):
    overlap = m - tau
    # tomar los PRIMEROS overlap índices de la ventana nueva: {0,1,...,overlap-1}
    sub = [pos for pos in pattern if pos < overlap]
    return relabel(sub)

def border_last(pattern, m, tau):
    overlap = m - tau
    # tomar los ÚLTIMOS overlap índices de la ventana anterior: {tau, ..., m-1}
    # y desplazarlos a 0..overlap-1 para que comparen contra el 'first' de la siguiente
    sub = [pos - tau for pos in pattern if pos >= tau]
    return relabel(sub)

def build_transition_graph(m, tau=3):
    overlap = m - tau
    pats = [tuple(p) for p in permutations(range(m))]
    idx = {p:i for i,p in enumerate(pats)}
    adj = [[] for _ in pats]
    if overlap <= 0:
        # sin solapamiento: todas las transiciones son posibles (no hay restricción ordinal)
        for i in range(len(pats)):
            adj[i] = list(range(len(pats)))
        return pats, idx, adj

    for i, p in enumerate(pats):
        bp = border_last(p, m, tau)
        for j, q in enumerate(pats):
            if border_first(q, m, tau) == bp:
                # print(border_first(q, m, tau), bp, p, q)
                adj[i].append(j)
    return pats,idx, adj
...
def simulate_op_smm(
    ...
    pats_all, idx_all, adj_all = build_transition_graph(m, tau = tau)
