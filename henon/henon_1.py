...
for r,a,tau in zip(r_values,a_values,range(1,301)):
    ...
    medida.append(ml.indice_S_eff_fast(x,seriey=None,tau=1, delta=False))
    # x,y = np.random.permutation(x), np.random.permutation(y)
    # x,y = ml.iaaft(np.array(x),1,0.1)[0], ml.iaaft(np.array(y),1,0.1)[0]
    x = ml.iaaft(np.array(x),1,0.1)[0]
    medida_null.append(ml.indice_S_eff_fast(x,seriey=None,tau=1,null="no",delta=False))
